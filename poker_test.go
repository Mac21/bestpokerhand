...
func TestTwoPairBeatsTwoPairBoardPaired(t *testing.T) {
    // Hand1 8cqd has two pair 8c8h & 7h7d
    // Hand2 2s4s has two pair 7h7d & 4s4d
    // result should be hand1 > hand2
    // it is currently treating hand1 as a single pair
	deck := buildBoardAndHand("7h2c4d7d8h8cqd2s4s")
	board := deck.DealCards(5)
	winning := deck.DealCards(2)
	losing := deck
	validateHandStrength(board, winning, losing, t)
